...
class OfflineSyncQueue {
  ...
  final Future<void> Function(String deliveryId, DeliveryStatus status) _applyCallback;

  Timer? _syncTimer;

  OfflineSyncQueue({required Future<void> Function(String, DeliveryStatus) applyUpdate, required this.logService})
  ...
  void setOnline(bool online) {
    _isOnline = online;
    logService.add('SYSTEM', 'NETWORK', online ? 'online' : 'offline', 'Network ${online ? 'restored' : 'lost'}');
    if (online) sync();
  }

  void enqueue(String deliveryId, DeliveryStatus status) {
    // Avoid duplicate same-status entries (idempotency)
    if (_queue.any((q) => q.deliveryId == deliveryId && q.status == status)) return;
  ...
  Future<void> sync() async {
    ...
    while (_queue.isNotEmpty) {
      ...
      try {
        ...
        logService.add(item.deliveryId, 'QUEUE', item.status.toString(), 'Synced');
      } catch (e) {
        logService.add(item.deliveryId, 'QUEUE', item.status.toString(), 'Sync failed: $e');
